...
- Debug HUD (`#status` + `#fpsHud`) defaults hidden so play UI stays clean; `?debug=1` / F3 toggles without touching gameplay HUD. VE marks use `#persistMark` (always available under `?ve=`). `ve/` is gitignored — `git add -f ve/babylon-*.png` so PR raw.githubusercontent.com embeds resolve.
