...
- Floater readability (#59): thicker dark outline + soft drop shadow, matte tints (heal mint vs grass), capped unlit emissive — no forest light/fog edits; `?ve=floater-read` (`ve/babylon-floater-read.png`).
...
- Player HP / death/respawn: durable `Character.Hp`/`MaxHp` + dummy thorns on `ApplyDamage` (keeps Cast NPC-only so CombatSmoke stays green); `PendingPlayerRespawn` schedules yard-origin full-HP revive; Move/Cast gate on `Hp≤0`. Headless `PlayerHpSmoke`; Babylon self-frame HP + stronger death greyout + live respawn countdown + clearer death/respawn toasts + self thorns floaters + ghost robe tint while dead (`?ve=player-hp`, `?ve=death-ux`).
...
- Yard bandage consumable: `Character.HasYardBandage` + `BandageReadyAt`; `BuyYardBandage` (vendor XP) grants bag flag; `UseBandage` consumes for HP-only heal (`Bandage.HealAmount`) with own CD/combat-lock (`Bandage.CooldownMs` / `CombatLockMs`) distinct from Rest; headless `BandageSmoke`; Babylon **N** use + `?ve=bandage`.
...
- Hotbar affordances (client-only): empty slots 3–6 (dashed); `.disabled` STAFF badge when staff unequipped (wins over OOM); `.lowMana` OOM badge + blue tint when mana < cost; optional unknown when `knowsSpark`/`knowsEmberbolt` false; `?ve=hotbar` seeds STAFF+OOM+empty for proof.
- Hotbar affordance polish (#63): stronger empty (hollow dashed) vs STAFF (bronze hatch + 🔒 badge) vs OOM (cyan wash + ◆ badge); selected/casting chrome kept; `?ve=hotbar-afford` (`ve/babylon-hotbar-afford.png`).
- GCD/cast scrub readability (client-only): thicker `.slotSweep` / `.slotCast` with lit top edges + stronger contrast; `?ve=gcd` seeds mid-GCD + mid-Emberbolt cast fill (`ve/babylon-gcd.png`).
- GCD bar chrome (#117, client-only): cool blue/silver `#gcdBar` plate+fill + crisp label (no neon); ready mint-silver vs sweeping blue; distinct from cast amber / lockout crimson / XP amber; `?ve=gcd-read`.
- Cast feedback clarity (client-only, #14): taller glowing `#castBar` with spell name; CANCEL ↩ (amber refund) vs LOCKOUT ⊘ (crimson hard-interrupt) toast chrome; Rest enter/exit on `#selfFrame` + REST (not RATE) for already-full; `?ve=cast-feedback` (`ve/babylon-cast-feedback.png`).
...
- Hard interrupt threshold: `PlayerCombat.CastPushbackCount` increments on pushback; when count ≥ `Combat.CastPushbackHardAfter` (or remain < `CastHardInterruptRemainMs`), next non-lethal hit calls `InterruptWindupCast(refundMana: false)`. Headless `HardInterruptSmoke`; Babylon LOCKOUT toast `castHardInterrupt` (`?ve=hard-interrupt`). Keep `CastPushbackSmoke` / `CastCancelSmoke` / `CombatSmoke` / `ManaSmoke` green.
- Post-interrupt silence: hard interrupt sets `PlayerCombat.CastLockedUntil` (`Combat.CastSilenceMs`); `Cast` rejects with `"silenced"` until expiry (soft CancelCast/Move does not silence). Headless `CastSilenceSmoke`; Babylon toast `silenced` (`?ve=cast-silence`). Keep `HardInterruptSmoke` / `CastPushbackSmoke` / `CastCancelSmoke` / `CombatSmoke` / `ManaSmoke` green.
- Kick / Counterspell: `Kick(target)` hard-interrupts nearby windup + CastLockedUntil silence without DummyStrike; `KickSmoke`; `?ve=kick` / `?ve=counterspell` + SecondClient. Keep CastSilence/HardInterrupt/Combat/Mana smokes green.
- Stun / Bash: `Stun(target)` breaks windup without CastLockedUntil; sets `StunnedUntilMicros` so Move/Cast reject `"stunned"`; `StunSmoke`; `?ve=stun` / `?ve=bash`. Keep Kick/Combat/Mana/CastSilence green.
- Debug HUD (`#status` + `#fpsHud`) defaults hidden so play UI stays clean; `?debug=1` / F3 toggles without touching gameplay HUD. VE marks use `#persistMark` (always available under `?ve=`). VE hosted on `ve.sparkify.dev` (upload via `tools/scripts/ve-upload.sh`) — do **not** commit `ve/*.png` to git.
