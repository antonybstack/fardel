...
- **Cause:** Grounded remotes snap XZ each 20 Hz pose. Walk is driven by a hold so it does not restart between snaps. A stopped snapshot still waited out 150 ms, so leftover Walk played at 0 wish. `syncRemoteMeshes` also runs from VE timers with the same pose — treating `step==0` as stop plants Idle between snaps and Walk never reads.
