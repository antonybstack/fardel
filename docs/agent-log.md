...
- **Do this:** If `stunnedUntilMicros` is in the future, drop `npcWalkHold` and `setHumanoidGroundWalk(false)`. `?ve=stun-hold` mid-chase; persistMark `hold` + `Brigand`; drift / Walk = fail. Dummy trainer.
- **Seen in:** #487
