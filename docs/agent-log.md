...
- **Do this:** Split seats: `FARDEL_SECOND_WALK=1` patrols SW pads (Walk, no Cast). `FARDEL_SECOND_CAST=1` stands in dummy range and loops Emberbolt with no Move during windup (#403). `?ve=remote-two-clips` persistMark names both clips + `skinned` + `remotes 2`. Hide You; skip Death / T-POSE. Dummy trainer.
- **Seen in:** #481
