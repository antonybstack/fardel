...
- **Target frame + spell hotbar HUD (presentation):** bottom-center Spark (1) / Emberbolt (2) + empty slots 3–6; distinct **disabled** (STAFF / no staff) vs **OOM** (`lowMana`) vs empty dashed affordances; GCD sweep + Emberbolt cast fill; `?ve=hotbar` / `?ve=target-frame` (`ve/babylon-hotbar.png`)
