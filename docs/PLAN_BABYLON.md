...
- **Say rate-limit (module + presentation):** per-identity 1s min interval on `Say` (`Fardel.Shared.Chat.SayMinIntervalMs`); reject `"Say rate-limited"`; client toast kind `rate`; `tools/ChatSmoke` proves reject + post-window OK
