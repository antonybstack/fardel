...
- **Target frame / reticule contrast (#69):** gold `#targetFrame` border + fog-immune world reticule/marker so select reads instantly under #39 cyan fog; tab-target logic unchanged; `?ve=target-contrast` (`ve/babylon-target-contrast.png`)
- **Cast bar chrome readability (#74):** `#castBar` / `#gcdBar` + CANCEL≠LOCKOUT toast chrome fog-immune; cast UX unchanged; no lighting lock edits; `?ve=castbar-read` (`ve/babylon-castbar-read.png`)
- **GCD bar chrome readability (#117):** cool blue/silver `#gcdBar` plate+fill distinct from warm Emberbolt cast / crimson lockout / XP amber; ready vs sweeping crisp under #39 fog; CSS/`?ve=` only; `?ve=gcd-read`
...
- **HUD layout defaults (presentation):** `#hudBottomLeft` flex column — chat / loadout chips / self+keybind hint non-overlapping vs bottom-center hotbar at 1280×720 under #39 fog; `?ve=hud-layout` (VE via https://ve.sparkify.dev)
...
- **Stun / Bash (invent):** `Stun(target)` hard-CC + `StunnedUntilMicros` (distinct from silence); `StunSmoke`; `?ve=stun` / `?ve=bash` (`ve/babylon-stun.png`)
...
- **Vendor panel chrome (presentation):** dark translucent silver plate + muted bronze buy / mint sell rows under #39 fog; distinct from stall prop + bag; `?ve=vendor-panel`
- **System toast readability (presentation):** compact dark plate + crisp category borders (invite/XP/equip/death/respawn/connected) under #39 fog; no plastic glow; `?ve=toast-read`
