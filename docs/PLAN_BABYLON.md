...
- **Player HP / death-respawn (invent):** `Character.Hp`/`MaxHp`; dummy thorns on Spark/Emberbolt apply; death clears target + schedules yard respawn; Babylon self-frame HP bar + death greyout/toast; `tools/PlayerHpSmoke`; `?ve=player-hp` (`ve/babylon-player-hp.png`)
