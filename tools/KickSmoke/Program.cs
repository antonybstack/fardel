...
try
{
    ...
    var vx = vendor.X;
    var vz = vendor.Z;
    await ExpectKickNpcFail(connA, vendor.VendorId, "Invalid target", "vendor KickNpc");
    vendor = FindVendor(connA)!;
    if (MathF.Abs(vendor.X - vx) > 0.01f || MathF.Abs(vendor.Z - vz) > 0.01f)
    {
        Fail($"KickNpc moved vendor stall ({vx},{vz})->({vendor.X},{vendor.Z})");
        return;
    }
    Console.WriteLine($"vendor KickNpc reject OK id={vendor.VendorId}");
    ...
    await ExpectKickNpcFail(connA, corpseId, "Target dead", "Kind=3 corpse KickNpc");
    Console.WriteLine("Kind=3 corpse KickNpc Target dead OK");
    dummy = FindDummy(connA)!;
    if (dummy.Kind != Combat.NpcKindDummy || dummy.Hp != dummyHpBeforeCorpse)
    {
        Fail($"dummy trainer changed after corpse KickNpc kind={dummy.Kind} hp={dummy.Hp}");
        return;
    }
    Console.WriteLine("dummy trainer OK after vendor + corpse KickNpc");
